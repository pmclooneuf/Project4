...
class Cell:
    ...
    def draw(self):
        ...
        if self.value != 0:
            ...
            num_rect = num_surf.get_rect(center = (self.col*WIDTH//9 + (WIDTH//18), self.row*(WIDTH//9) + (WIDTH//18) + 5))
            self.screen.blit(num_surf, num_rect)
        if self.sketched != None and self.sketched != 0:
            num_font = pg.font.Font(None, 30)
            num_surf = num_font.render(str(self.sketched), 1, (100, 100, 100))
            num_rect = num_surf.get_rect(center = (self.col*WIDTH//9 + (WIDTH//18) - 15, self.row*(WIDTH//9) + (WIDTH//18) - 10))
            self.screen.blit(num_surf, num_rect)
